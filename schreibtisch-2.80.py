...
bpy.ops.mesh.primitive_cylinder_add(vertices=128,radius=3*(breite+tiefe)/math.pi,location=(5.5*tiefe,0,0))
...
bpy.ops.mesh.primitive_cylinder_add(vertices=128,radius=3*(breite+tiefe)/math.pi,location=(2.5*tiefe,0,-2))
bpy.ops.transform.resize(value=(1*tiefe, 2*breite, 1))
...
bpy.ops.mesh.primitive_cube_add(location = (9/2*tiefe-0.5-tiefe*2, (16/2-0.5)*breite, -4.29))
leg = scn.objects['Cube']
leg.name = 'Tischbein1'
leg.select_set(state=True)
bpy.ops.transform.resize(value=(0.5, 0.5, 4.5))
leg.select_set(state=False)
bpy.ops.mesh.primitive_cube_add(location = (-9/2*tiefe+0.5, (-16/2+0.5)*breite,-4.29))
leg = scn.objects['Cube']
leg.name = 'Tischbein2'
leg.select_set(state=True)
bpy.ops.transform.resize(value=(0.5, 0.5, 4.5))
...
"mirror":False, "proportional_edit_falloff":'SMOOTH', "proportional_size":1, "snap":False, "snap_target":'CLOSEST',
"snap_point":(0, 0, 0), "snap_align":False, "snap_normal":(0, 0, 0), "texture_space":False, "release_confirm":False,"use_proportional_connected":False,"use_proportional_projected":False,"orient_matrix_type":'GLOBAL',"orient_type":'GLOBAL'})
...
bpy.ops.mesh.primitive_cylinder_add(vertices=128,radius=5*(breite+tiefe)/math.pi,location=(5.5*tiefe-(9/2/2*tiefe)*2,0,3))
...
#    lightspheres.append((scn.objects[-1],(1,1,1, 1.0)))
    ...
    bpy.ops.mesh.primitive_cylinder_add(vertices=128,radius=3*(breite+tiefe)/math.pi,location=(-3.6*tiefe,a*breite,0))
...
set4anim(camd,cams,100.0*mult_frames,(cams.location[0]-5,-cams.location[1],3),(math.pi*1.4, -math.pi, math.pi*1.6-math.pi/4))
set4anim(lampd,lamps,100.0*mult_frames,(lamps.location[0],0,1),None)
set4anim(camd,cams,200.0*mult_frames,(cams.location[0]-10,0,-4),(math.pi*1.4, -math.pi, math.pi*1.6))
set4anim(lampd,lamps,200.0*mult_frames,(lamps.location[0]-4,-3,-4),None)
set4anim(camd,cams,300.0*mult_frames,(cams.location[0]-20,0,-5),(math.pi*1.4+math.pi/2, -math.pi, math.pi*1.6))
set4anim(lampd,lamps,300.0*mult_frames,(lamps.location[0]-5,0,-5),None)
set4anim(camd,cams,400.0*mult_frames,(cams.location[0]-20,0,18),(math.pi*1.4+math.pi/2+math.pi/3, 0, math.pi*1.6))
set4anim(lampd,lamps,350.0*mult_frames,(0,0,22),None)
set4anim(camd,cams,500.0*mult_frames,(0,-30,10),(math.pi*1.4+math.pi, 0, math.pi*1.6+math.pi/2))
set4anim(lampd,lamps,450.0*mult_frames,(0,10,22),None)
set4anim(camd,cams,600.0*mult_frames,(30,-20,0),(math.pi*1.4+math.pi, 0, math.pi*1.6+math.pi/2+math.pi/4))
set4anim(camd,cams,700.0*mult_frames,(7, 0, 27),(math.pi*1.4+math.pi-math.pi/4, 0, math.pi*1.6+math.pi/2+math.pi/4))
set4anim(lampd,lamps,800.0*mult_frames,(10, 10, 10),None)
set4anim(camd,cams,800.0*mult_frames,(30, 10, 7),(math.pi*1.4, -math.pi, math.pi*1.6))
set4anim(camd,cams,900.0*mult_frames,(12.3, 23.8, 7),(math.pi*1.4, -math.pi, math.pi*1.72))
set4anim(camd,cams,1000.0*mult_frames,(4, 16, 13),(math.pi*1.26, -math.pi*1.11, math.pi*1.7))
set4anim(camd,cams,1100.0*mult_frames,(11.1, -5, 3.3),(math.pi*1.48, -math.pi*1.02, math.pi*1.42))
set4anim(camd,cams,1200.0*mult_frames,(24, -9.76, -2),(math.pi*1.48, -math.pi*1.02, math.pi*1.42))
set4anim(camd,cams,1300.0*mult_frames,(30, 10, 7),(math.pi*1.4, -math.pi, math.pi*1.6))
...
bpy.data.materials['Concrete 02'].node_tree.nodes["Mapping"].inputs[3].default_value[0]=15
bpy.data.materials['Concrete 02'].node_tree.nodes["Mapping"].inputs[3].default_value[1]=15
bpy.data.materials['Concrete 02'].node_tree.nodes["Mapping"].inputs[3].default_value[2]=15
...
#scn.objects['davor.001'].select_set(state=True)
#cont = bpy.context
#print(str(cont)+'\n'+str(dir(cont)))
#bpy.context.scene.objects.active = scn.objects['davor']
#bpy.context.window = None
#bpy.context.screen = None
#bpy.context.view_layer.objects.active = scn.objects['davor.001']
#bpy.ops.object.mode_set(mode = 'EDIT')
...
def createPlane(name,size=1.0,location=(0.0,0.0,0.0),rotation=(0.0,0.0,0.0)):
    bpy.ops.mesh.primitive_plane_add(size=2.0, calc_uvs=True, enter_editmode=False, align='WORLD', location=location, rotation=rotation)
    ...
    for a in [60-0.01-8.8]:
        for i in range(-11,11):
            for k in range(-14,14):
                kacheln.append(createPlane("kachel-"+str(a)+str(i)+str(k),20,(4*i,4*k,a)))

    for a in [39.9,-39.9]:
        for i in range(-10,10):
            for k in range(-14,14):
                kacheln.append(createPlane("kachel-"+str(a)+str(i)+str(k),20,(a,4*k,-8.8+25+i*4),rotation=(0,math.pi/2,0)))

    for a in [49.9,-49.9]:
        for i in range(-10,10):
            for k in range(-14,14):
                kacheln.append(createPlane("kachel-"+str(a)+str(i)+str(k),20,(4*k,a,-8.8+25+i*4),rotation=(0,math.pi/2,math.pi/2)))
...
bpy.data.materials['Concrete Tiles'].node_tree.nodes["Mapping"].inputs[3].default_value[0] = 15
bpy.data.materials['Concrete Tiles'].node_tree.nodes["Mapping"].inputs[3].default_value[1] = 15
bpy.data.materials['Concrete Tiles'].node_tree.nodes["Mapping"].inputs[3].default_value[2] = 15
makeKacheln()
